		...
		(function($, undefined) {
			...
			$.fn.jstree = function(arg) {
				...
				this.each(function() {
					...
					// result - break and return the value
			...
			$.jstree.core.prototype = {
				...
				bind : function() {
					...
					this.element.on("dblclick.jstree", function(e) {
						...
						e.preventDefault(); // prevent any node inside
						// from losing focus when
						// clicking the open/close
						// icon
						 ...
						 * !== $.jstree.root) { o = this.is_selected(o) ?
					...
					.on(
							...
							$.proxy(function(e) {
								...
								var chr = String.fromCharCode(e.which).toLowerCase(), col = this.element.find('.jstree-anchor').filter(
				 ...
				 */
				_redraw : function() {
					var nodes = this._model.force_full_redraw ? this._model.data[$.jstree.root].children.concat([]) : this._model.changed
				...
				redraw_node : function(node, deep, is_callback, force_render) {
					...
					node = !document.querySelector ? document.getElementById(obj.id) : this.element[0].querySelector('#'
							+ ("0123456789".indexOf(obj.id[0]) !== -1 ? '\\3' + obj.id[0] + ' '
									+ obj.id.substr(1).replace($.jstree.idregex, '\\$&') : obj.id.replace($.jstree.idregex, '\\$&'))); // ,
					// this.element);
					if (!node) {
						deep = true;
						// node = d.createElement('LI');
						if (!is_callback) {
							par = obj.parent !== $.jstree.root ? $('#' + obj.parent.replace($.jstree.idregex, '\\$&'), this.element)[0]
									: null;
					...
					if (!this.is_loaded(obj)) {
						...
						if (d.length) {
							...
							if (!animation) {
								...
								d.children(".jstree-children").css("display", "none").end().removeClass("jstree-closed").addClass(
										"jstree-open").attr("aria-expanded", true).children(".jstree-children").stop(true, true).slideDown(
										animation, function() {
											this.style.display = "";
											if (t.element) {
				...
				close_node : function(obj, animation) {
					...
					if (!d.length) {
						...
						if (!animation) {
							...
							d.children(".jstree-children").attr("style", "display:block !important").end().removeClass("jstree-open")
									.addClass("jstree-closed").attr("aria-expanded", false).children(".jstree-children").stop(true, true)
				...
				activate_node : function(obj, e) {
					...
					if (!this.settings.core.multiple
						...
						if (e.shiftKey) {
							...
							for (i = 0, j = p.length; i < j; i += 1) {
								// separate IFs work whem o and l are the
								// same
								if (p[i] === o) {
									c = !c;
								}
				...
				set_icon : function(obj, icon) {
					...
					if (icon === false) {
						...
						dom.removeClass(old).css("background", "");
						dom.addClass('jstree-themeicon-custom').css("background", "url('" + icon + "') center center no-repeat").attr(
								"rel", icon);
						if (old === false) {
							this.show_icon(obj);
						}
			 ...
			 * ### Changed plugin
			 * 
			 * This plugin adds more information to the `changed.jstree` event.
			 * The new data is contained in the `changed` event data property,
			 * and contains a lists of `selected` and `deselected` nodes.
			 */
														...
														function(e, data) {
															...
															if (s.indexOf('down') !== -1 && dom.length) {
																dom.find('.jstree-anchor')
																		.addClass(t ? 'jstree-clicked' : 'jstree-checked').parent().attr(
																				'aria-selected', true);
															}
														}, this))
														...
														function(e, data) {
															...
															if (s.indexOf('up') !== -1) {
																for (i = 0, j = obj.parents.length; i < j; i++) {
																	...
																	if (tmp && tmp.length) {
																		tmp.attr('aria-selected', false).children('.jstree-anchor')
															...
															// separately?)
															if (s.indexOf('down') !== -1 && dom.length) {
																dom.find('.jstree-anchor').removeClass(
																		t ? 'jstree-clicked' : 'jstree-checked').parent().attr(
																		'aria-selected', false);
															}
														}, this));
					}
			...
			$.jstree.defaults.contextmenu = {
				 ...
				 * same structure as $.jstree.defaults.contextmenu.items which
				 * can be used to create a submenu - each key will be rendered
				 * as a separate option in a submenu that will appear once the
			...
			$.jstree.plugins.contextmenu = function(options, parent) {
				this.bind = function() {
					 ...
					 * null; this.element .on("touchstart", ".jstree-anchor",
													...
													function(e) {
														...
														if (obj
															...
															if (this.settings.dnd.use_html5) {
																...
																return $.vakata.dnd.start(e, drg, '<div id="jstree-dnd" class="jstree-'
																		+ this.get_theme()
																		+ ' jstree-'
																		+ this.get_theme()
																		+ '-'
			...
			$(function() {
				...
				var lastmv = false, laster = false, lastev = false, opento = false, marker = $('<div id="jstree-marker">&#160;</div>')
						.hide(); // .appendTo('body');
								...
								function(e, data) {
									...
									if (ins && ins._data && ins._data.dnd) {
										...
										if ((data.event.target === ins.element[0] || data.event.target === ins.get_container_ul()[0])
											...
											ref = ins.settings.dnd.large_drop_target ? $(data.event.target).closest('.jstree-node')
													.children('.jstree-anchor') : $(data.event.target).closest('.jstree-anchor');
												...
												$
														.each(
																...
																function(j, v) {
																	...
																	if (v === 'i' && ref.parent().is('.jstree-closed')
																	...
																	if (ok) {
																		...
																		data.helper.find('.jstree-icon').first().removeClass('jstree-er')
																				.addClass('jstree-ok');
			...
			$.jstree.defaults.types = {
				...
				"n_ngi_group" : {
					"icon" : "fa fa-map-o",
					"valid_children" : [ "n_ngi_layer_pt", "n_ngi_layer_ln", "n_ngi_layer_pg", "n_ngi_layer_mpt", "n_ngi_layer_mln",
							"n_ngi_layer_mpg", "n_ngi_layer_txt" ]
				},
				"n_dxf_group" : {
					"icon" : "fa fa-map-o",
			...
			$.jstree.plugins.wholerow = function(options, parent) {
				this.bind = function() {
					...
					this.element.on('ready.jstree set_state.jstree', $.proxy(function() {
						...
						// div.style.height = this._data.core.li_height
						// + 'px';
